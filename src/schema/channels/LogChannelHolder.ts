...
class LogChannelHolder {
  ...
  static async createLogChannelHolderDocument(
    categoriesRef: Ref<GuildCategory>[]
  ): Promise<LogChannelHolder> {
    // Populate all the categories and within them all channels.
    const categories = await GuildCategoryModel.find({
      _id: { $in: categoriesRef }
    }).populate('channels');
    ...
    const abstractChannels = categories
      .flatMap(category => category.channels)
      .filter((channel): channel is Ref<AbstractChannel> => channel !== undefined);
